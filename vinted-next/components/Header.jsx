...
const Header = () => {
    const { user, logout, mode, toggleMode, setShowLoginModal } = useContext(AuthContext);
    ...
    const handleImageChange = async (e) => {
        ...
        try {
            ...
            if (res.data.query) {
                ...
                router.push(`/products?search=${encodeURIComponent(res.data.query)}`);
            ...
            if (error.response?.status === 503) {
                ...
                router.push('/products?ai_setup=required');
        ...
        <header className="header-container" onMouseLeave={handleCategoryBarLeave}>
            <div className="header-content container-fluid">
                <div className="logo-section">
                    <Link href="/" className="site-logo" style={{ color: settings.primary_color, cursor: 'pointer', userSelect: 'none' }} onClick={handleCategoryBarLeave}>
                        {settings.site_logo ? (
                            <img
                                    ...
                                    {recommendedSuggestions.length > 0 ? (
                                        recommendedSuggestions.map((item, idx) => (
                                            <div
                                                key={`rec-${idx}`}
                                                onClick={() => {
                                                    router.push(`/products?category=${item.catSlug}&subcategory=${item.subSlug}`);
                                                    closeAllDropdowns();
                    ...
                    <div className="icon-group">
                        ...
                        {user ? (
                                    ...
                                    {isNotifDropdownOpen && (
                                        <div style={{
                                            ...
                                            <div style={{
                                                ...
                                                <div
                                                    onClick={() => { router.push('/profile?tab=notifications'); setIsNotifDropdownOpen(false); }}
                                                    style={{
                                    ...
                                    {isUserDropdownOpen && (
                                        <div className="user-dropdown-wrapper" style={{
                                            ...
                                            <div className="user-dropdown-box" style={{
                                                ...
                                                {mode === 'buyer' ? (
                                                    <>
                                                        <Link href="/profile?tab=orders&mode=buyer" className="dropdown-item-custom" style={{ display: 'flex', alignItems: 'center', gap: '12px' }} onClick={() => setIsUserDropdownOpen(false)}>
                                                        ...
                                                        <Link href="/profile?tab=favorites&mode=buyer" className="dropdown-item-custom" style={{ display: 'flex', alignItems: 'center', gap: '12px' }} onClick={() => setIsUserDropdownOpen(false)}>
                                                        ...
                                                        <Link href="/profile?tab=messages&mode=buyer" className="dropdown-item-custom" style={{ display: 'flex', alignItems: 'center', gap: '12px' }} onClick={() => setIsUserDropdownOpen(false)}>
                                                    ...
                                                    <>
                                                        <Link href="/profile?tab=listings&mode=seller" className="dropdown-item-custom" style={{ display: 'flex', alignItems: 'center', gap: '12px' }} onClick={() => setIsUserDropdownOpen(false)}>
                                                        ...
                                                        <Link href="/profile?tab=payments&mode=seller" className="dropdown-item-custom" style={{ display: 'flex', alignItems: 'center', gap: '12px' }} onClick={() => setIsUserDropdownOpen(false)}>
                                                        ...
                                                        <Link href="/profile?tab=messages&mode=seller" className="dropdown-item-custom" style={{ display: 'flex', alignItems: 'center', gap: '12px' }} onClick={() => setIsUserDropdownOpen(false)}>
                                                            <div style={{ width: '20px', display: 'flex', justifyContent: 'center' }}><FaBell style={{ color: '#adb5bd', fontSize: '1rem' }} /></div>
                                                            {t('user_menu.messages', 'Messages')}
                                                        </Link>
                                                        <div style={{ borderTop: '1px solid #f1f3f5', margin: '4px 0' }}></div>
                            ...
                            <div className="auth-split-button" style={{ border: 'none' }}>
                                <Link href="/login" className="auth-split-link">{t('header.login', 'Login')}</Link>
                                <span className="auth-split-divider">|</span>
                                <Link href="/register" className="auth-split-link">{t('header.signup', 'Sign Up')}</Link>
                            </div>
                        )}
                            ...
                            <div className="mega-menu-right">
                                ...
                                {activeSubcategory ? (
                                    <div className="mm-item-grid">
                                        ...
                                        <Link
                                            href={`/products?category=${activeCategory.slug}&subcategory=${activeSubcategory.slug}`}
                                        ...
                                        {activeSubcategory.items && activeSubcategory.items.length > 0 && activeSubcategory.items.map(item => (
                                            <Link
                                                key={item._id}
                                                href={`/products?category=${activeCategory.slug}&subcategory=${activeSubcategory.slug}&itemType=${item.slug}`}
                                                className="mm-item-link"
                                                onClick={() => setShowCategoryBar(false)}
                                            >
                ...
                <div className="mo-content">
                    ...
                    {mobileLevel === 1 && mobileSelectedCategory && (
                        <div>
                            ...
                            <div className="mo-item" onClick={() => { closeMobileMenu(); router.push(`/products?category=${mobileSelectedCategory.slug}`); }}>
                    ...
                    {mobileLevel === 2 && mobileSelectedSubcategory && (
                        <div className="item-list-container">
                            ...
                            <div className="mo-item" onClick={() => { closeMobileMenu(); router.push(`/products?category=${mobileSelectedCategory.slug}&subcategory=${mobileSelectedSubcategory.slug}`); }}>
                            ...
                            {mobileSelectedSubcategory.items?.map(item => (
                                <div key={item._id} className="mo-item" onClick={() => { closeMobileMenu(); router.push(`/products?category=${mobileSelectedCategory.slug}&subcategory=${mobileSelectedSubcategory.slug}&itemType=${item.slug}`); }}>
