...
const instance = axios.create({
    baseURL: process.env.NEXT_PUBLIC_API_BASE_URL === '/' ? '' : (process.env.NEXT_PUBLIC_API_BASE_URL || 'https://vinted.sangvish.com'),
});
            ...
            if (error.config.url.includes('/api/admin')) {
                ...
                if (!isLoginRequest) {
                    ...
                    if (authHeader && authHeader.startsWith('Bearer ') && authHeader.length > 15) {
                        ...
                        const isProtected = typeof window !== 'undefined' && protectedPaths.some(p => window.location.pathname.startsWith(p));
