...
// @desc    Upload image for frontend content
// @route   POST /api/frontend-content/admin/upload-image
// @access  Private (Admin)
export const uploadFrontendImage = asyncHandler(async (req, res) => {
    if (!req.file) {
        res.status(400);
        throw new Error('No file uploaded');
    }

    const url = `images/site/${req.file.filename}`;
    res.json({ url });
});
