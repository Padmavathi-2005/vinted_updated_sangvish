...
// Find the mobile-toggle comment and replace the block
...
if (startIndex !== -1) {
    ...
    // Replace lines from startIndex up to where the next non-empty block starts (Horizontal Category Bar)
    const endIndex = lines.findIndex(l => l.includes('{/* Horizontal Category Bar */}'));
    if (endIndex !== -1) {
