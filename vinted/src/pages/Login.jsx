...
const Login = () => {
    ...
    const [recaptchaSettings, setRecaptchaSettings] = useState({ 
        recaptcha_enabled: import.meta.env.VITE_RECAPTCHA_ENABLED === 'true', 
    ...
    useEffect(() => {
            ...
            try {
                ...
                if (recaptchaRes.data) {
                    setRecaptchaSettings(prev => ({
                        ...prev,
                        ...recaptchaRes.data,
                        // Prioritize the correct env key if dynamic one is missing/wrong
                        recaptcha_site_key: import.meta.env.VITE_RECAPTCHA_SITE_KEY || recaptchaRes.data.recaptcha_site_key
                    }));
                }
            } catch (err) {
        ...
        if (socialToken) {
            // Success: Log the user in with the token from the URL
            login({ token: socialToken }, rememberMe);
            navigate('/');
        } else if (socialError) {
            // Failure: specific error (like Blocked) from the provider
